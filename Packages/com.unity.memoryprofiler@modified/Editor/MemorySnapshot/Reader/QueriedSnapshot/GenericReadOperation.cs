...
namespace Unity.MemoryProfilerExtension.Editor.Format.QueriedSnapshot
{
    public struct ReadOperation : IDisposable
    {
        public JobHandle JobHandle => m_Handle.JobHandle;
        ReadHandle m_Handle;
        ReadError m_Err;
        DynamicArray<byte> m_Buffer;
        public ReadOperation(ReadHandle handle, DynamicArray<byte> buffer)
        {
    ...
    public interface IGenericReadOperation : IDisposable
    {
        ReadError Error { get; }
    ...
    public unsafe struct GenericReadOperation : IGenericReadOperation, IDisposable
    {
        ReadOperation ReadOperation;
        public GenericReadOperation(ReadOperation readOperation)
        {
            ReadOperation = readOperation;
        }
        public GenericReadOperation(ReadHandle handle, DynamicArray<byte> buffer) : this(new ReadOperation(handle, buffer)) { }

        public ReadError Error { get => ReadOperation.Error; set => ReadOperation.Error = value; }
    ...
    public unsafe struct NestedDynamicSizedArrayReadOperation<T> : IGenericReadOperation where T : unmanaged
    {
        bool m_DoneReading;
        ...
        public NestedDynamicSizedArrayReadOperation(List<GenericReadOperation> genericReadOperations, NestedDynamicArray<T> nestedArrayStructure, AllocatorType allocator)
        {
        ...
        public NestedDynamicArray<T> CompleteReadAndGetNestedResults()
        {
            if (!m_DoneReading)
            {
                if (!IsDone && IsCreated)
