...
{
    public class AllTrackedMemoryTableViewController : TreeViewController<AllTrackedMemoryModel, AllTrackedMemoryModel.ItemData>, IAnalysisViewSelectable
    {
        const string k_UxmlAssetGuid = "04f67e30fbf7b984495ddc016db517d2";
        ...
        public override ToolbarSearchField SearchField => m_SearchField;
        ...
        public void TryExcuteAfterLoading(Action action, int waitFrames)
        {
            if (waitFrames > 0) {
                // wait frames for LoadingOverlay show
                m_TreeView.schedule.Execute(() => TryExcuteAfterLoading(action, waitFrames - 1));
            }
            else if (m_LoadingOverlay.visible) {
                // wait loading
                m_TreeView.schedule.Execute(() => TryExcuteAfterLoading(action, 0));
            }
            else {
                action();
            }
        }
        public void TryExpandAndSelect(Queue<int> expandIds, int selectedId, int waitFrames)
        {
            m_TreeView.schedule.Execute(() => TryExpandAndSelectAsyncRecursively(expandIds, selectedId, waitFrames));
        }
        void TryExpandAndSelectAsyncRecursively(Queue<int> expandIds, int selectedId, int waitFrames)
        {
            if (waitFrames > 0) {
                // wait frames for LoadingOverlay show
                m_TreeView.schedule.Execute(() => TryExpandAndSelectAsyncRecursively(expandIds, selectedId, waitFrames - 1));
            }
            else if (m_LoadingOverlay.visible) {
                // wait loading
                m_TreeView.schedule.Execute(() => TryExpandAndSelectAsyncRecursively(expandIds, selectedId, 0));
            }
            else if (expandIds.Count > 0) {
                int id = expandIds.Dequeue();
                TryExpandTreeViewItem(id);
                m_TreeView.schedule.Execute(() => TryExpandAndSelectAsyncRecursively(expandIds, selectedId, 0));
            }
            else {
                TrySelectAndExpandTreeViewItem(selectedId);
            }
        }
        bool TryExpandTreeViewItem(int itemId)
        {
