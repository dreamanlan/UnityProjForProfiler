...
{
    public unsafe interface ILongIndexedContainer<T> : IEnumerable<T>
        where T : unmanaged
    {
    ...
    public unsafe readonly struct DynamicArrayRef<T> : IEquatable<DynamicArrayRef<T>>, ILongIndexedContainer<T> where T : unmanaged
    {
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public unsafe struct DynamicArray<T> : ILongIndexedContainer<T>, IDisposable where T : unmanaged
    {
        [NativeDisableUnsafePtrRestriction]
        T* m_Data;
        long m_Capacity;
