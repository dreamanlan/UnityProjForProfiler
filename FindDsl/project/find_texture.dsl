...
filter
{
    if(stringcontains(assetpath, filter) && stringnotcontains(assetpath, notfilter) && (filterMode=="Any" || importer.filterMode==parseenum("FilterMode", filterMode))){
        ...
        var(6) = gettexturesetting("Android");
    		...
    		if((var(1) > maxSize || var(2) > maxSize) && (var(5).maxTextureSize > maxSize || var(6).maxTextureSize > maxSize) && (prop.Contains("1") && var(3) || !prop.Contains("1")) && (prop.Contains("2") && var(4) || !prop.Contains("2")) && (!notastc || textureisnotastc(var(5)) || textureisnotastc(var(6)))){
